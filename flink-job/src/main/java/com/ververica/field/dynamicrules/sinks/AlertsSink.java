...
public class AlertsSink {
  ...
  public static <Event, Value> DataStream<String> alertsStreamToJson(
      DataStream<Alert<Event, Value>> alerts) {
    return alerts
        .flatMap(new JsonSerializer<>(alerts.getType().getTypeClass(), log))
